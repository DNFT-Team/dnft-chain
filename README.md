...
```bash
RUST_LOG=debug RUST_BACKTRACE=1 ./target/release/dnft -lruntime=debug --dev
```
...
also replace the default command (`cargo build --release && ./target/release/dnft --dev --ws-external`)
